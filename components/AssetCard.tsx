...
export function AssetCard({
  ...
  const isGeneratingPreview =
    activeVersion &&
    activeVersion.file_path && // Has background
    !activeVersion.preview_file_path && // No preview yet
    !activeVersion.preview_generation_failed; // Not failed

  // Check if preview generation failed
  const previewFailed = activeVersion?.preview_generation_failed === true;
    ...
    >
      ...
      <div className={`relative bg-bg-secondary border border-border-primary rounded-xl overflow-hidden transition-all duration-300 hover:border-border-secondary hover:shadow-lg ${
        ...
        <div className="relative aspect-[9/16] bg-gradient-to-br from-bg-tertiary to-bg-secondary overflow-hidden">
          ...
          {isGeneratingPreview && (
            <div className="absolute bottom-3 right-3 bg-yellow-500/90 text-black text-xs px-2 py-1 rounded shadow-lg">
          ...
          {previewFailed && (
            <div className="absolute bottom-3 right-3 bg-red-500/90 text-white text-xs px-2 py-1 rounded shadow-lg">
              <div className="flex items-center gap-1">
        ...
        <div className="p-4 space-y-3">
          ...
          <div className="flex items-center gap-2 text-xs text-fg-tertiary">
            ...
            <span>
              {asset.status === 'Scheduled' && asset.scheduled_time ? (
                <>
                  <span className="font-medium">Scheduled:</span>{' '}
                  {format(new Date(asset.scheduled_time), 'MMM dd, yyyy HH:mm')}
                </>
              ) : asset.status === 'Published' && asset.scheduled_time ? (
                <>
